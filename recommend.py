...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min(centroids, key=lambda tup: distance(location, tup))
    # END Question 3
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    xs_mean = mean(xs)
    xs_list = [(i - xs_mean) ** 2 for i in xs]
    S_xx = sum(xs_list)

    ys_mean = mean(ys)
    ys_list = [(i - ys_mean) ** 2 for i in ys]
    S_yy = sum(ys_list)

    xs_list_two = [i - xs_mean for i in xs]
    ys_list_two = [i - ys_mean for i in ys]
    xsys_list = [a * b for a, b in zip(xs_list_two,ys_list_two)]
    S_xy = sum(xsys_list)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    list_of_pred_rsquared = []
    for feature_fn in feature_fns:
        predicator_function, r_squared = find_predictor(user, reviewed, feature_fn)
        list_of_pred_rsquared.append([predicator_function, r_squared])
    max_r_squared = 0
    max_fn = feature_fns[0]
    if len(list_of_pred_rsquared) > 0:
        for elem in list_of_pred_rsquared:
            r_squared = elem[1]
            fn = elem[0]
...
def search(query, restaurants):
    ...
    list_of_restaurants = []
    for restaurant in restaurants:
        restaurant_category = restaurant_categories(restaurant)
        if query in restaurant_category:
